...
class Lectures_model extends CI_Model {
    ...
    public function get( $value ) {
        $this->db->from($this->table);
        $this->db->group_start()->or_where(['lct_id' => $value, 'lct_name' => $value])->group_end();
        $query = $this->db->get();
    ...
    public function update( $id, $data ) {
        return $this->_update($this->table, ['lct_id' => $id], $data);
    }

    public function delete( $id ) {
        return $this->_delete($this->table, ['lct_id' => $id]);
    }
    
    public function get_onDate()
    {
        $this->db->select('lct_id,lct_title,day(lct_date),lct_description,lct_file,lct_description');
        $this->db->from($this->table);
        $query=$this->db->get();
        return $query->row_array();
    }
